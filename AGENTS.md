...
- `www/` — actual app source (no build step)
  - `index.html`, `style.css`, `app.js` (drag-and-drop syllable game), `sfx.js` (short synthesized sound effects, no voice), `words.js` (160 words with syllable splits + `image` icon path), `home.js` (map/trail + parents area), `progress.js` (localStorage progress)
  - `assets/images/words/*.png` — 158 stylized transparent icons, one per unique word
...
- Local dev (any OS): `python -m http.server 8000` or `npx serve .` Serving from `localhost`/HTTPS is recommended; opening `index.html` via `file://` may still hit browser restrictions on module-less script loading in some setups.
...
- **Browser requirement for the app itself:** any modern browser with Pointer Events support (Chrome, Edge, Firefox, Safari, desktop or mobile). No voice recognition or narration is used — the game is drag-and-drop only, with short synthesized sound effects (`sfx.js`, Web Audio API). Works fully offline.
- **Required runtime permissions on Android:** `INTERNET` only (already declared in `android/app/src/main/AndroidManifest.xml`). No microphone permission is needed.
